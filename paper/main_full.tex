...
Across the six pairs with at least 12 shared labels, raw dollar-cost Spearman correlation ranges from 0.74 to 0.93, with mean 0.84. These associations survive the pre-specified raw-cost multiplicity and permutation checks. However, the pooled cost-propensity model explains 82.0\% of log-cost variation (adjusted \(R^2=72.7\%\)). After removing benchmark and displayed-label cost propensity, no high-overlap residual-cost rank association is significant under a 20,000-draw permutation test. The result is not that cost is irrelevant; it is that raw cost portability largely reflects a stable observed label-level component rather than demonstrated transferable trajectory behavior.
...
Across the same six high-overlap pairs, mean raw-cost rank correlation is 0.84 whereas mean CPS rank correlation under the 1\% floor is 0.59. CPS portability declines in five pairs. GAIA--SWE-mini changes from 0.89 for raw cost to 0.29 for CPS; CORE--SWE-mini changes from 0.74 to 0.32; SWE-mini--TAU changes from 0.82 to 0.45. CORE--TAU is an exception, with CPS 0.83 versus raw cost 0.78. Thus a label that is predictably cheap is not necessarily predictably cheap per expected success on a different workload.
