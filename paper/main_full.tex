...
\item a matched external trace extension showing substantial scaffold differences in turns and tool calls at fixed task and model, and showing that the popular ``expensive failure'' premium is not detected under exact task matching in this setting; and
...
\citet{kapoor2026hal} introduce \hal{} as cost-aware infrastructure across coding, web, science, and customer-service tasks. \citet{agentsmatter2024} motivate joint accuracy--cost reporting and convex-hull frontiers. \citet{ndzomga2026efficient} ask whether task subsets preserve ranks within agent benchmarks. Benchmark$^2$ studies cross-benchmark rank consistency for static LLM benchmarks. SWE-Effi examines resource effectiveness within software engineering and reports that failed attempts consume several times the resources of successful ones; our matched design tests that heuristic directly and finds no failure premium at fixed task (Section~\ref{sec:failure-burden}). Open-SWE-Traces releases public agent trajectories~\citep{sweeffi2025,ahmad2026openswetraces}. Our contribution is a coverage-aware test of whether a public cost signal predicts a held-out interactive-agent workload, whether that signal survives success adjustment, and what matched traces can reveal about scaffold burden and failure-cost composition.
...
\subsection{Failure carries no burden premium at fixed task}
\label{sec:failure-burden}

The same sample tests a popular cost heuristic: comparing each scaffold's failed versus resolved trajectories across tasks, failures look 19--36\% heavier (turns ratio 1.21 [1.07,1.36] OpenHands, 1.19 [1.07,1.35] SWE-agent; trajectory characters up to 1.33), the ``expensive failure'' premium reported in prior resource-effectiveness work. Exact task matching does not detect this premium. Among the 107 discordant pairs --- identical task, identical model, one scaffold resolved, the other failed --- the failed side is heavier only 47--52 times out of 107 on every metric (two-sided binomial \(p\geq0.25\)), and after normalizing each trajectory by its own scaffold's median burden, the failed/resolved median ratio is 0.98--1.06 with sensitivity intervals spanning 1 throughout (Figure~\ref{fig:failure-burden}). With 107 pairs, a two-sided binomial test at \(\alpha=0.05\) has 80\% power to detect a premium fraction of 0.65 or larger (failed heavier on 70+ pairs); the observed 0.47--0.53 falls well below this threshold, ruling out strong versions of the failure-cost claim but not modest ones. This analysis uses one model (MiniMax-M2.5) and a nonrandom four-shard sample, so it tests the failure heuristic in this setting rather than establishing a general absence. Hard tasks make agents both burn more and fail; unconditioned failure-cost ratios therefore measure workload composition, not a property of failing runs. Failure-rate-times-premium accounting inherits exactly the composition problem documented for raw aggregate dollars in Section~\ref{sec:primary-results}.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/failure_burden_discordant.pdf}
\caption{The failure-cost premium is a composition artifact. Left: across tasks, failed trajectories appear 20--33\% heavier (bootstrap intervals). Right: on the 107 matched pairs where scaffolds disagree on the identical task, the failed side is heavier about half the time, raw or scaffold-normalized, consistent with no premium.}
\label{fig:failure-burden}
\end{figure}
...
The combined results clarify what a cost-aware agent leaderboard can and cannot establish. Raw dollar-cost rankings are predictive for some held-out workloads, but their portability is largely associated with stable observed displayed-label cost propensity. CPS portability is weaker, so raw cheapness is not interchangeable with deployment efficiency. Independent matched traces further show that scaffolds change measurable execution structure, even when task and model are fixed. The failure-burden analysis completes a single composition theme across all three studies: every unconditioned aggregate we can compute from public data --- raw cost ranks, success-adjusted efficiency, and failure premiums --- conflates which workloads were attempted with how agents behaved on them. In the failure case, the composition is not noise around a recoverable signal; it is the dominant component in each test we ran, though this conclusion is demonstrated for one model and a nonrandom sample, and modest premiums cannot be excluded.
...
Observed dollar-cost portability is real in several high-overlap agent benchmark pairs, but it should not be read as a universal deployment-efficiency signal. It is partly persistent label-level cost propensity, weakens after success adjustment, and coexists with substantial scaffold differences in matched trajectory structure --- while the failure-cost premium is not detected under exact task matching in this setting, though modest premiums cannot be excluded. Agent leaderboards should report not only accuracy and raw dollar cost, but also success-adjusted outcomes and trace-level usage fields needed to distinguish price, scaffold, workload composition, and execution mechanisms.
...
\textbf{Scope.} This appendix is a supplementary descriptive audit of public unencrypted telemetry corpora. It does not test the paper's portability, cost-propensity, or failure-burden claims. Its purpose is to characterize what observable token, cache, and tool-level anatomy public data can supply, and where the measurement gaps remain.
...
The MIMO corpus replicates the error cascade at an independent site and model: the next tool call errs 4.3\% of the time after an error versus 1.0\% after a success, a \(4.33\times\) conditional increase that closely mirrors the TraceLab ratio, with the same directional burden gradient across error-count bins (median session input 0.40M, 0.94M, and 1.12M tokens at zero, one, and two to five errors). Two audit boundaries apply. First, in every released MIMO session the per-round usage block repeats one constant tuple (identical uncached input, cache creation, cache read, and output across all rounds), so within-session token dynamics cannot be validated in that source; we therefore use it only for session counts, corpus totals, cross-session concentration (top 10\% of sessions hold 37.9\% of summed input), and content-derived error sequences. The constant-usage issue means the source's per-round fields cannot be independently verified and should not be treated as high-fidelity round-level telemetry. Second, neither source links usage to task success or billing, so these are composition and association findings, not failure-cost measurements.
